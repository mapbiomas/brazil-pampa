...
var regioes = [2,5,6,7//3,5,7//1,3, 4, 5, 7
                ]
...
//Map.addLayer(dir_filtros, {}, 'dir_filtros', false);
...
for (var i_ano=0;i_ano<anos.length; i_ano++){
  ...
  var class_ano = image_in.select('classification_'+ano)
...
var nivel0_ultimo = class_nivel0.select('classification_2025')
var nivel0_penultimo = class_nivel0.select('classification_2024')

//IMPEDE FALSO DESMATAMENTO 
// corrige desmatamentos pequenos no último ano
var desmat = nivel0_ultimo.eq(10).and(nivel0_penultimo.eq(1))//antropico e natural
...
var desmat1ha = conectedDesmat.lte(22)// 2 ha
var ruido_desmat_penultimo = image_in.select('classification_2024').updateMask(desmat1ha)//aqui ele copia os pixels do penultimo ano para aplicar no ultimo

// impede FALSA REGENERAçÃO
...
var ruido_regen_penultimo = image_in.select('classification_2024').updateMask(regen1ha)
...
var ruido_desmat85 = image_in.select('classification_1986').updateMask(desmat1ha)
...
var conectedregen = regen.selfMask().connectedPixelCount(30,true).reproject('epsg:4326', null, 30);
var regen1ha = conectedregen.lte(22)
var ruido_regen85 = image_in.select('classification_1986').updateMask(regen1ha)
...
// Map.addLayer(image_in, vis2, 'col9', true);
...
// var class_previous = class_final.select('classification_2024')
...
var class_previous = class_final.select('classification_2024')
var class_last = class_final.select('classification_2025')

var mask_floresta = class_previous.neq(3).and(class_last.eq(3)).selfMask() //mascara de não floresta em 23 e floresta em 24
var mask_umida = class_previous.neq(11).and(class_last.eq(11)).selfMask() //mascara de não umica em 23 e umida em 24

var corr_floresta = class_previous.mask(mask_floresta) //camada que classificação de 2023 onde havia floresta em 24
var corr_umida = class_previous.mask(mask_umida) //camada que classificação de 2023 onde havia area umida em 24

var class_last_corr = class_last.blend(corr_floresta).blend(corr_umida) //camada que classificação de 2024 com camadas de floresta e umida

Map.addLayer(class_last_corr, vis, 'class_2025_corr', true);

class_final = class_final
  .addBands(class_last_corr,['classification_2025'],true)
print(class_final)

Map.addLayer(class_final.select('classification_2025'), vis, 'class_2025_corr_final', true);
...
var class_1995 = class_final.select('classification_1995')
var class_2003 = class_final.select('classification_2003')

var anv_1995 = class_1995.eq(22).selfMask().multiply(22) //mascara de anv em 2001
Map.addLayer(anv_1995, {}, 'anv_1998', true);

var class_2003_corr = class_2003.blend(anv_1995) //camada de 2023 corrigida
