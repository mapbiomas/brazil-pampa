...
var imageVisParam = {"opacity":1,"bands":["con_ano_area"],"min":772.6135864257812,"max":77271.234375,"palette":[]};
...
var regioesCollection = ee.FeatureCollection('projects/mapbiomas-workspace/AUXILIAR/REGIOES/VETOR/Pampa_Reg6_AB_buff')
var reg = regioesCollection.filter(ee.Filter.eq('name', '6A'));
Map.addLayer(reg)
...
for (var i_regiao=0;i_regiao<regioes.length; i_regiao++){
  ...
  var image_in =  ee.Image(dir_filtros+ '06' +'_Emb_RF17a25_v' + versionIn).clip(reg);
                                      ///0               1     _Emb_RF17a24_v   03_esp_fim
Map.addLayer(image_in)
  ...
  for (var i_ano=0;i_ano<anos.length; i_ano++){
   ...
   // Map.addLayer(ano_21,{'min': 0, 'max': 62,'palette': palettes.get('classification8')}, "ano_21",false);
    ...
    var obj = con_ano.reduceToVectors({
  ...
  */
  ...
  ' PAMPA_obj_' + ano + '_r' + regiao + '_v' + version, 
   'projects/mapbiomas-workspace/AMOSTRAS/S2_Coll04/PAMPA/SEGMENTS/obj_class_' 
